...
namespace SPBot.Core
{
    public class CryptocurrencyModule : ModuleBase
    {
        ...
        {
            string ApiEndpoint = "https://api.kucoin.com/v1/open/currencies?coins=" + SearchQuery.ToUpper();
            using (WebClient WC = new WebClient())
            {
                ...
                {
                    string USDValue = Token.SelectToken("data.rates.DRGN.USD").ToString();
                    string GBPValue = Token.SelectToken("data.rates.DRGN.GBP").ToString();
